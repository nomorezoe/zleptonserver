...
function FluxPipeTextToTightColor(task, req, queue) {
    console.log("FluxPipeTextToTightColor");
    var session = req.body.session;
    var text = req.body.prompt;

    const promptFile = fs.readFileSync('./pipe/flux_text_2_tight_color.json');//');
    let prompt = JSON.parse(promptFile);

    prompt["6"]["inputs"]["text"] = "Servstyle, " + text;
    ...
    task.pipeline = "flux_text_to_tight_color";
    ...
    const reqhttps = https.request(options, (reshttps) => {
        ...
        if (reshttps.statusCode == 200) {
            ...
            reshttps.on('end', (d) => {
                ...
                console.log("onend_flux_txt_to_tc: " + task.key + " , time: ");
                for (var i = 0; i < jsonobj.length; i++) {

                    var styleImageName = uuidv4() + "_flux_txt_to_tc.png";
                    console.log("flux_txt_to_tc:" + styleImageName);
...
module.exports = FluxPipeTextToTightColor;