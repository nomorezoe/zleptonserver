...
function TaskComfyInPaint(task, req, queue) {
    ...
    switch (inpaintStyle) {
        ...
        case "3":
            ...
            break;
        case "4":
            task.pipeline = "inpaint_flux";
            promptjson = TaskComfyInPaint.fluxProcess(req.body.fullFilePath, req.body.tags, maskBytes, prompt);
...
TaskComfyInPaint.fluxProcess = function (fullFilePath, tags, maskBytes, prompt) {
    const promptFile = fs.readFileSync('./pipe/flux_inpaint.json');
    let promptjson = JSON.parse(promptFile);

    Tool.applyImage(promptjson, "1495", null, fullFilePath);
    promptjson["1499"]["inputs"]["image"] = maskBytes;

   //promptjson["1496"]["inputs"]["width"] = dWidth;
    //promptjson["1496"]["inputs"]["height"] = dHeight;

    promptjson["1489"]["inputs"]["text"] = prompt;
    promptjson["1484"]["inputs"]["noise_seed"] = Tool.randomInt();
    return promptjson;
}
